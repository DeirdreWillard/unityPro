...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed partial class DebuggerComponent : GameFrameworkComponent
    {
        ...
        private sealed class ConsoleWindow : IDebuggerWindow
        {
			...
			async void UploadLog() {
				using UnityWebRequest request = UnityWebRequest.Post("http://192.168.31.226:25001/loginfo", new WWWForm());
				...
				if (request.result != UnityWebRequest.Result.Success) {
					...
					DebuggerComponent.CopyToClipboard("上传失败");
					return;
				}
				LogInfo response = JsonUtility.FromJson<LogInfo>(request.downloadHandler.text);
				int count = 0;
				List<LogNode> nodes = new(); nodes.AddRange(m_LogNodes);
				foreach (LogNode node in nodes) {
					WWWForm nodeinfo = new();
					nodeinfo.AddField("uuid", response.uuid);
					nodeinfo.AddField("time", node.LogTime.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss"));
					nodeinfo.AddField("framecount", node.LogFrameCount.ToString());
					nodeinfo.AddField("type", node.LogType.ToString());
					nodeinfo.AddField("message", node.LogMessage);
					nodeinfo.AddField("stacktrack", node.StackTrack);
					using UnityWebRequest noderequest = UnityWebRequest.Post("http://192.168.31.226:25001/lognode", nodeinfo);
					UnityWebRequestAsyncOperation nodeoperation = noderequest.SendWebRequest();
					while (!nodeoperation.isDone) {
						await Task.Yield();
					}
					if (noderequest.result != UnityWebRequest.Result.Success) continue;
					count++;
				}
				Debug.Log($"<color=#F22E2E> Upload. uuid: {response.uuid}. size: {nodes.Count} count: {count} </color>");
				DebuggerComponent.CopyToClipboard(response.uuid);
			}
            ...
            public void OnDraw()
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    if (GUILayout.Button("Upload", GUILayout.Width(100f)))
                    {
						UploadLog();
                    }
                }
