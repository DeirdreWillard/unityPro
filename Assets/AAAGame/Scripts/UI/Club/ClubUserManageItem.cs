...
public class ClubUserManageItem : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void Init(LeagueUser leagueUser)
    {
        ...
        transform.Find("Bg/BtnKickOut").gameObject.SetActive(!Util.IsMySelf(leagueUser.PlayerId));
        transform.Find("Bg/ButtonCoinOp").gameObject.SetActive(!Util.IsMySelf(leagueUser.PlayerId));
        transform.Find("Bg/BtnNotes").gameObject.SetActive(!Util.IsMySelf(leagueUser.PlayerId));

        //userIdtxt.text = leagueUser.PlayerId.ToString();
        //userNametxt.text= leagueUser.NickName.ToString();

        // itemTransform.localPosition = new Vector3(leftPositionX, itemTransform.localPosition.y, itemTransform.localPosition.z);
        MoveToLeft();
    }
