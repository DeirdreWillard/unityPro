...
public partial class AlliancesDetailPanel : UIFormBase
{
    ...
    public bool CheckSuperAlliance()
    {
        //加入了超级联盟
        if (leagueInfo.Type == 1 && leagueInfo.Father != 0)
        {
            GF.UI.ShowToast("请退出超级联盟恢复功能");
            return true;
        }
        return false;
    }
    ...
    protected override async void OnButtonClick(object sender, string btId)
    {
        ...
        switch (btId)
        {
            ...
            case "ManagerList":
                ...
                break;
            case "BtnQuitUnion":
                if (CheckSuperAlliance()) return;
                Util.GetInstance().OpenConfirmationDialog("解散联盟", "确定要解散联盟", () =>
                   {
                       GF.LogInfo("解散联盟");
            ...
            case "SuperAlliance":
                if (leagueInfo.Type == 1 && leagueInfo.Father == 0)
                    await GF.UI.OpenUIFormAwait(UIViews.JoinSuperAlliance, uiParams);
                else
