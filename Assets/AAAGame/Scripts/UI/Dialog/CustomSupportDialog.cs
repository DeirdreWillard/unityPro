...
public class CustomSupportDialog : UIFormBase
{
    public Image header;
    public GameObject[] objectSupportGroup;
    public Image[] imageSupportGroup;
    public Text[] textSupportGroup;
    ...
    IEnumerator getSupportInfo()
    {
        ...
        objectSupportGroup[0].SetActive(false);
        objectSupportGroup[1].SetActive(false);
        ...
        if (request1.result == UnityWebRequest.Result.Success)
        {
            ...
            {
                objectSupportGroup[1].SetActive(true);
                textSupportGroup[1].text = csinfo.qq;
            }
            //wecaht 代替 telegam
            if (csinfo.telegram != null && csinfo.telegram != "")
            {
                objectSupportGroup[0].SetActive(true);
                textSupportGroup[0].text = csinfo.telegram;
            }
            if (csinfo.jspp != null && csinfo.jspp != "")
            {
                objectSupportGroup[2].SetActive(true);
                textSupportGroup[2].text = csinfo.jspp;
            }
            ...
            if (csinfo.image != null && csinfo.image != "")
            {
                ...
                if (request2.result == UnityWebRequest.Result.Success)
                {
                    ...
                    header.sprite = Sprite.Create(texture, new Rect(0, 0, texture.width, texture.height), Vector2.zero);
