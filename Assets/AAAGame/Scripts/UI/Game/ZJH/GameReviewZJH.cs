...
public class GameReviewZJH : MonoBehaviour
{
	...
	private Msg_ZjhLastHistoryRs LastHistoryRs;

	void OnEnable()
	{
		HotfixNetworkComponent.AddListener(MessageID.Msg_ZjhLastHistoryRs, Function_ZjhLastHistoryRs);
	...
	void OnDisable()
	{
		HotfixNetworkComponent.RemoveListener(MessageID.Msg_ZjhLastHistoryRs, Function_ZjhLastHistoryRs);
	...
	public void Function_ZjhLastHistoryRs(MessageRecvData data)
	{
		Msg_ZjhLastHistoryRs ack = Msg_ZjhLastHistoryRs.Parser.ParseFrom(data.Data);
	...
	public void ReshUI()
	{
		int round = totalRound - curPage;
		textPage.text = round + "/" + totalRound;

		ZJHProcedure zjh_procedure = GF.Procedure.CurrentProcedure as ZJHProcedure;
		bool isMustShowCard = zjh_procedure.enterZjhDeskRs.Config.SysShowCard;
		...
		for (int i = 0; i < LastHistoryRs.History.Count; i++)
		{
			...
			child.GetComponent<GameReviewZJHItem>().Init(LastHistoryRs.History[index]);
			child.GetComponent<GameReviewZJHItem>().lookCardAction = () =>
			{
				_ = GF.Sound.PlayEffect(AudioKeys.SOUND_BTN);
				Util.GetInstance().OpenConfirmationDialog("查看手牌", $"确定要消耗<color=#FFFF00>{GlobalManager.GetConstants(15)}</color>欢乐豆查看玩家手牌", () =>
				{
					Msg_LookCardRq req = MessagePool.Instance.Fetch<Msg_LookCardRq>();
					req.Round = round;
					req.PlayerId = LastHistoryRs.History[index].BasePlayer.PlayerId;
	...
	public void Function_LookCard(MessageRecvData data)
	{
		...
		GF.LogInfo("收到玩家手牌. ", ack.ToString());
		if (ack.Round != totalRound - curPage) return;
		foreach (Transform item in Content)
		{
			if (item.name == ack.PlayerId.ToString())
			{
				item.GetComponent<GameReviewZJHItem>().ShowCard(ack.HandCard.ToArray(), ack.CardType);
				break;
			}
		}
	}
