...
namespace UGUI.Extend
{
    ...
    public class ColorFader : BaseMeshEffect
    {
        ...
        private void DrawVertical(VertexHelper vh, List<UIVertex> vertices, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (isFlip)
                {
                    color = Color32.Lerp(color1, color2, 1 - (vertex.position.y - bottomY) / height * (1f - range));
                }
                else
                {
                    color = Color32.Lerp(color1, color2, (vertex.position.y - bottomY) / height * (1f - range));
                }
        ...
        private void DrawHorizontal(VertexHelper vh, List<UIVertex> vertices, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (isFlip)
                {
                    color = Color32.Lerp(color1, color2, 1 - (vertex.position.x - leftX) / width * (1f - range));
                }
                else
                {
                    color = Color32.Lerp(color1, color2, (vertex.position.x - leftX) / width * (1f - range));
                }
