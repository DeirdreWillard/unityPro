...
public class PreloadProcedure : ProcedureBase
{
    ...
    protected override void OnUpdate(IFsm<IProcedureManager> procedureOwner, float elapseSeconds, float realElapseSeconds)
    {
        ...
        if (loadAsset == true)
        {
            ...
            {
                deltaTime = 0;
                showAsset = showAsset == 1 ? 2 : 1;
            }
            GF.BuiltinView.SetLoadingProgress((float)loadedProgress / (float)totalProgress, showAsset);
        }
        else
        {
            GF.BuiltinView.SetLoadingProgress(smoothProgress, showAsset);
        }
        //预加载完成 切换场景
        if (loadedProgress >= totalProgress && smoothProgress >= 0.99f)
        {
            ...
            ChangeState<StartProcedure>(procedureOwner);
        }
    }
    ...
    private async void PreloadAndInitData()
    {
        ...
        CreateGFExtension();
        // 初始化SoundManager
        GF.Sound.InitializeSoundManager();
    }
    ...
    private void OnLoadDicSuccess(object sender, GameEventArgs e)
    {
...
#if !UNITY_EDITOR
        GF.BuiltinView.SetVersionText(GameFramework.Version.GameVersion, GFBuiltin.Resource.InternalResourceVersion);
#endif
        ...
        {
            loadAsset = true;
            List<UITable> needPreloadUIs = GF.DataTable.GetDataTable<UITable>().Where(t => t.Preload == true).ToList();
            loadedProgress = 0;
            smoothProgress = 0;
            totalProgress = needPreloadUIs.Count;

            // 并行加载所有UI预制体
            PreloadUIPrefebs(needPreloadUIs);
        }
        else
        {
            loadedProgress++;
    ...
    /// 并行预加载UI预制体
    /// </summary>
    /// <param name="_needPreloadUIs">需要预加载的UI列表</param>
    private async void PreloadUIPrefebs(List<UITable> _needPreloadUIs)
    {
        // 创建UniTask任务列表
        List<UniTask> tasks = new List<UniTask>();
        
        // 每个预制体创建一个异步加载任务
        foreach (UITable table in _needPreloadUIs)
        {
            string path = UtilityBuiltin.AssetsPath.GetUIFormPath(table.UIPrefab, table.Path);
            GF.LogInfo($"开始预加载UI预制体:{path}");
            
            // 使用UniTask创建异步加载任务
            var task = CreateLoadPrefabTask(path);
            tasks.Add(task);
        }
        
        // 分批执行加载任务，每批次最多4个任务并行执行，避免内存峰值过高
        const int batchSize = 4;
        for (int i = 0; i < tasks.Count; i += batchSize)
        {
            int count = Math.Min(batchSize, tasks.Count - i);
            await UniTask.WhenAll(tasks.GetRange(i, count));
            GF.LogInfo($"预加载批次完成: {i/batchSize + 1}/{(tasks.Count + batchSize - 1)/batchSize}");
        }
        
        GF.LogInfo("所有UI预制体预加载完成!");
        loadedProgress = totalProgress; // 标记为全部完成
    }

    /// <summary>
    /// 创建单个预制体的加载任务
    /// </summary>
    private UniTask CreateLoadPrefabTask(string _path)
    {
        UniTaskCompletionSource<bool> tcs = new UniTaskCompletionSource<bool>();
        
        GF.Resource.LoadAsset(_path, new LoadAssetCallbacks(
            (assetName, asset, duration, userData) =>
            {
                GF.LogInfo($"预加载UI预制体成功: {_path}，耗时: {duration}秒");
                loadedProgress++;
                showAsset = 1;
            ...
            {
                GF.LogWarning($"预加载UI预制体失败: {_path}，错误: {errorMessage}");
                loadedProgress++;
                showAsset = 1;
