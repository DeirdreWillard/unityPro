                ...
                if (resourceRule.valid)
                {
                    switch (resourceRule.filterType)
                    {
                        ...
                        case ResourceFilterType.ChildrenFoldersOnly:
                            {
                                // 递归获取所有子文件夹(包括深层子文件夹)
                                DirectoryInfo rootDir = new DirectoryInfo(resourceRule.assetsDirectoryPath);
                                var allDirectories = new List<DirectoryInfo>();
                                
                                // 递归收集所有子文件夹
                                void CollectSubDirectories(DirectoryInfo parentDir)
                                {
                                    try
                                    {
                                        ...
                                        foreach (DirectoryInfo subDir in subDirs)
                                        {
                                            ...
                                            CollectSubDirectories(subDir); // 递归处理子文件夹
                                        }
                                    }
                                    catch (System.Exception ex)
                                    {
                                        Debug.LogWarning($"无法访问文件夹 {parentDir.FullName}: {ex.Message}");
                                    }
                                }
                                
                                // 先添加根目录,再收集所有子文件夹
                                allDirectories.Add(rootDir);
                                CollectSubDirectories(rootDir);

                                // 检查文件夹是否包含符合条件的文件(不包括子文件夹中的文件)
                                bool HasValidFiles(DirectoryInfo dir)
                                {
                                    string[] patterns = resourceRule.searchPatterns.Split(';', ',', '|');
                                    foreach (string pattern in patterns)
                                    {
                                        ...
                                        {
                                            Debug.LogWarning($"无法访问文件夹 {dir.FullName}: {ex.Message}");
                                            continue;
                                        }
                                ...
                                // 递归检查文件夹及其所有后代是否包含文件
                                bool HasFilesRecursive(DirectoryInfo dir)
                                {
                                    // 先检查当前文件夹是否有文件
                                    if (HasValidFiles(dir))
                                        return true;
                                ...
                                // 对根目录和所有子文件夹应用资源过滤,每个文件夹创建独立的 AssetBundle
                                foreach (DirectoryInfo directory in allDirectories)
                                {
                                    // 只处理包含文件的文件夹,跳过只有子文件夹的空文件夹
                                    if (!HasValidFiles(directory))
                                        continue;
                                    
                                    string relativeDirectoryName =
                                        directory.FullName.Substring(Application.dataPath.Length + 1);
                                    string resourceName = Utility.Path.GetRegularPath(relativeDirectoryName);
                                    
                                    // 检查是否有子文件夹(包括深层子文件夹)也包含文件,如果有则添加后缀避免冲突
                                    bool hasSubDirWithFiles = false;
                                    try
                                    {
                                    ...
                                    // 只有当目录本身有文件且子目录也有文件时,才添加后缀避免冲突
                                    if (hasSubDirWithFiles)
                                    {
                                        resourceName += "_folder";
                                    }

                                    // 每个文件夹作为独立的 AssetBundle,SearchOption.TopDirectoryOnly 只打包当前文件夹内的文件
                                    ApplyResourceFilter(SearchOption.TopDirectoryOnly, ref signedAssetBundleList, resourceRule,
                                        resourceName, string.Empty, directory.FullName);
                                }
                            }
                            break;
