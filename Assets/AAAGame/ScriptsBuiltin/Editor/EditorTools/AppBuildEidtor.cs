...
namespace UGF.EditorTools
{
    ...
    public class AppBuildEidtor : EditorWindow
    {
        ...
        /// <summary>
        /// Jenkins 构建完成回调 (success, outputPath)
        /// </summary>
        public static Action<bool, string> OnJenkinsBuildComplete;
        
        /// <summary>
        /// 是否在 Jenkins 模式下运行
        /// </summary>
        private bool m_IsJenkinsMode = false;
        ...
        private GUIContent hotfixUrlFallbackContent;
        ...
        {
            hotfixUrlContent = new GUIContent("Update Prefix Uri", "主资源服务器地址（优先使用）");
            hotfixUrlFallbackContent = new GUIContent("Fallback Uri", "备用资源服务器地址（主地址失败时使用）");
        ...
        {
            ObfuzSettings.Instance.buildPipelineSettings.enable = AppBuildSettings.Instance.EnableObfuz;
        ...
        private void DrawHotfixConfigPanel()
        {
            ...
            EditorGUILayout.BeginVertical("box");
            {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    EditorGUILayout.LabelField(hotfixUrlFallbackContent, GUILayout.Width(160f));
                    AppBuildSettings.Instance.UpdatePrefixUriFallback = EditorGUILayout.TextField(AppBuildSettings.Instance.UpdatePrefixUriFallback);
                }
                EditorGUILayout.EndHorizontal();
        ...
        private void OnPostprocessBuild(BuildReport report)
        {
            ...
            if (report.summary.result != BuildResult.Succeeded)
            {
                ...
                // Jenkins 模式下触发回调
                if (m_IsJenkinsMode && OnJenkinsBuildComplete != null)
                {
                    OnJenkinsBuildComplete.Invoke(false, outputPath);
                    OnJenkinsBuildComplete = null;
                }
                return;
            }
            ...
            if (m_IsJenkinsMode && OnJenkinsBuildComplete != null)
            {
                OnJenkinsBuildComplete.Invoke(true, outputPath);
                OnJenkinsBuildComplete = null;
            }
        }
        ...
        private void BuildResources()
        {
            if (m_Controller.BuildResources())
            {
                ...
                Debug.Log($"Resource version auto-incremented to: {m_Controller.InternalResourceVersion}");
                SaveConfiguration();
            }
            else
            {
        ...
        /// Jenkins打包资源调用
        /// </summary>
        public bool JenkinsBuildResource(JenkinsBuildResourceConfig configJson)
        {
            ...
            // Jenkins模式下直接同步构建，不使用m_OrderBuildResources异步方式
            m_Controller.OutputPackedSelected = (AppSettings.Instance.ResourceMode != ResourceMode.Package) && HasPackedResource();
            
            // 刷新 ResourceCollection.xml，确保使用最新的资源规则配置
            Debug.Log("Refreshing ResourceCollection.xml...");
            ResourceTools.ResourceRuleEditorUtility.RefreshResourceCollection();
        ...
        public void JenkinsBuildApp(JenkinsBuildAppConfig configJson)
        {
            ...
            if (tPlatform == Platform.Undefined)
            {
                ...
                OnJenkinsBuildComplete?.Invoke(false, null);
                OnJenkinsBuildComplete = null;
                return;
            }
